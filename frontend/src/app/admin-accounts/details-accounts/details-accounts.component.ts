...
export class DetailsAccountsComponent {
  ...
  constructor(
    ...
    this.balanceForm = this.fb.group({
      ...
      balance: [
        data?.balance || 0,
        [Validators.required, Validators.min(0), this.decimalMaxTwoValidator],
      ],
    });
  }
  ...
  onSave() {
    ...
    this.accountService.updateUserBalance(email, newBalance).subscribe({
      next: (response) => {
        this.snackBar.open(
          (response as any).message || "Balance updated successfully",
          "Close",
          {
            duration: 3000,
      ...
      error: (error) => {
        this.snackBar.open(
          error.error.error || "Failed to update balance",
          "Close",
          {
            duration: 3000,
            verticalPosition: "top",
